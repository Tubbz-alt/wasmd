...
TEST_DOCKER_REPO=jackzampolin/xrntest
...
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/xrnd/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/xrnd:Z tendermint/xrndnode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
...
start-xrn: setup-contract-tests-data
	./build/xrnd --home /tmp/contract_tests/.xrnd start &
	@sleep 2s

setup-transactions: start-xrn
	@bash ./lcd_test/testdata/setup.sh

run-lcd-contract-tests:
	@echo "Running Regen LCD for contract tests"
...
.PHONY: all build-linux install format lint \
	...
	setup-transactions setup-contract-tests-data start-xrn run-lcd-contract-tests contract-tests \
	test test-all test-build test-cover test-unit test-race \
	benchmark \
	build-docker-xrndnode localnet-start localnet-stop \
	docker-single-node
